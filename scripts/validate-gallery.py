...
This template is *delisted*: metadata.yaml was deleted on purpose (e64c746). The
script therefore runs in two modes, keyed off whether metadata.yaml exists — see
LISTED below. Delisted, the rules that only bind a published template are reported as
warnings; the LICENSE and ___INFO___ rules still fail the build, and the full contract
re-arms automatically if the template is ever resubmitted.
...
# This template was delisted from the gallery: metadata.yaml was deleted on purpose
# (e64c746), which is the sanctioned way to remove a template. Rules that only bind a
# *listed* template are therefore reported as warnings rather than failures, so the
# guard still catches a LICENSE or ___INFO___ regression today and re-arms in full the
# moment metadata.yaml comes back.
LISTED = METADATA_PATH.is_file()
...
def check_git_available() -> bool:
    """Preflight: git must work before any other check is meaningful.

    The file-tracking and versions[] checks all shell out to git, and every one of
    them reads a failure as a contract violation. Without this gate, running the
    script outside a checkout (or on a machine with no git) reports a screenful of
    invented violations instead of the one real problem.
    """
    result = git("rev-parse", "--git-dir")
    if result is None:
        fail(
            "git-required",
            "git is not available on PATH. It is needed for the file-tracking and "
            "version checks.",
        )
        return False
    if result.returncode != 0:
        fail(
            "git-required",
            "not inside a git repository. Run this script from the repository "
            f"checkout ({result.stderr.strip() or 'git rev-parse failed'}).",
        )
        return False
    return True
...
def check_required_files() -> None:
    ...
    for path in (LICENSE_PATH, TEMPLATE_PATH):
        if not path.is_file():
    ...
    if not LISTED:
        warn(
            "required-files",
            "metadata.yaml is absent, so the template is delisted from the gallery. "
            "That is the intended state for this deprecated repository; restore the "
            "file only as part of a deliberate resubmission.",
        )
    ...
    #
    # Every check below reads `tracked`. An empty list because git failed looks
    # exactly like "nothing is tracked", which would report a fleet of misleading
    # violations, so bail out loudly instead of guessing. check_git_available() has
    # already run, so this only catches a failure specific to `ls-files`.
    listing = git("ls-files")
    if listing.returncode != 0:
        fail(
            "required-files",
            f"`git ls-files` failed: {listing.stderr.strip() or 'unknown error'}. "
            "The file-tracking checks could not run.",
        )
        return
    ...
    # A file that exists locally but is untracked never reaches the gallery, which
    # only ever sees what GitHub serves from the default branch.
    if TEMPLATE_PATH.is_file() and "template.tpl" not in tracked:
        fail(
            "required-files",
            "template.tpl exists locally but is not tracked at the repository root, "
            "so it would not ship",
        )
...
def check_template_info() -> None:
    ...
    if categories is None:
        # A listed template without categories is rejected; a delisted one simply
        # never had them. Report it either way so a resubmission starts informed.
        report = fail if LISTED else warn
        report(
            "template-categories",
...
def main() -> int:
    # Hard gate: without git, every check below reports noise rather than signal.
    ...
    if LISTED:
        ...
        print(
            "OK  delisted template; every gallery rule that still applies is satisfied"
        )
    return 0
